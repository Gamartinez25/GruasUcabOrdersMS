...
{
    public class ListarResumenOrdenesHandler : IRequestHandler<ListarResumenOrdenesQuery, List<ResumenOrdenDto>>
    {
        private readonly IOrdenRepository OrdenRepository;
        private readonly IGoogleService GoogleService;
        public ListarResumenOrdenesHandler(IOrdenRepository ordenRepository, IGoogleService googleService)
        {
            OrdenRepository = ordenRepository;
            GoogleService = googleService;
        }
        public async Task<List<ResumenOrdenDto>> Handle(ListarResumenOrdenesQuery request, CancellationToken cancellationToken)
        {
            var ordenes = await OrdenRepository.GetAllOrdenAsync();
            var ordenesPorGruero = ordenes.Where(x => x.Vehiculo == request.IdVehiculo);
            List<ResumenOrdenDto> ordenesResumen = new List<ResumenOrdenDto>();
            foreach (var orden in ordenesPorGruero)
            {
                ...
                if (estatus.EstadoActual != "Cancelado")
                {
                    ...
                    ordenesResumen.Add(resumenOrden);
                }

            }
            return ordenesResumen;
        }
    }
}
